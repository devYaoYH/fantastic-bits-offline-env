...
public class Simulator extends MultiReferee {
    ...
    public Simulator(InputStream is, PrintWriter out, PrintStream err) throws IOException {
        super(is, out, err);
    }
    ...
    static class Vector {
        ...
        private Vector symmetricRound() {
            return new Vector(Simulator.symmetricRound(this.x), Simulator.symmetricRound(this.y));
        }
    ...
    abstract static class DynamicEntity extends Entity {
        ...
        @Override
        public String toPlayerString() {
            return this.id + " " + this.type + " " + Math.round(position.x) + " " + Math.round(position.y) + " " + Simulator.symmetricRound(speed.x) + " " + Simulator.symmetricRound(speed.y) + " " + 0;
        }
    ...
    class Pod extends DynamicEntity {
        ...
        public String toPlayerString(boolean myPlayer) {
            ...
            return this.id + " " + type + " " + positionX + " " + positionY + " " + Simulator.symmetricRound(speed.x) + " " + Simulator.symmetricRound(speed.y) + " " + ((this.snaffle != null) ? 1 : 0);
        }

        public String toViewString() {
            StringBuilder pods = new StringBuilder();
            pods.append(Math.round(position.x) + " " + Math.round(position.y) + " " + Simulator.symmetricRound(speed.x) + " " + Simulator.symmetricRound(speed.y) + " " + power);
...
abstract class MultiReferee extends AbstractReferee {
    ...
    public MultiReferee(InputStream is, PrintWriter out, PrintStream err) throws IOException {
        super(is, out, err);
    }
    ...
    protected void appendDataToEnd(PrintWriter stream) throws IOException {
...
abstract class SoloReferee extends AbstractReferee {
    ...
    public SoloReferee(InputStream is, PrintWriter out, PrintStream err) throws IOException {
        super(is, out, err);
    }
    ...
    protected void appendDataToEnd(PrintWriter stream) {
    }
...
abstract class AbstractReferee {
    ...
    protected abstract void appendDataToEnd(PrintWriter stream) throws IOException;
